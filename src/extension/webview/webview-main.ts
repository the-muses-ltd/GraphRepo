import { GraphRenderer } from "../../web/graph-renderer.js";
import {
  fetchGraph,
  searchCode,
  fetchNodeDetails,
...
const typeFilters = document.querySelectorAll<HTMLInputElement>(
  '#type-filters input[type="checkbox"]'
);
const relFilters = document.querySelectorAll<HTMLInputElement>(
  '#rel-filters input[type="checkbox"]'
);
...
renderer.setOnNodeClick(async (node: GraphNode) => {
  ...
  try {
    ...
    if (details.relationships?.length > 0) {
      html += `<div style="margin-top: 6px"><span class="detail-key">Connections:</span></div>`;
      for (const rel of details.relationships) {
        if (!rel.relType) continue;
        const arrow = rel.direction === "out" ? "\u2192" : "\u2190";
        html += `<div style="padding-left: 8px; color: var(--vscode-descriptionForeground, #aaa)">${arrow} ${rel.relType} ${rel.relatedName ?? ""}</div>`;
      }
    }
...
searchInput.addEventListener("input", () => {
  ...
  debounceTimer = setTimeout(async () => {
    ...
    try {
        ...
        .map(
          (r) =>
            `<div class="search-result-item" data-id="${r.id}">
              <span class="dot" style="background: ${getTypeColor(r.type)}"></span>
              ${r.name}
              <span style="color: var(--vscode-descriptionForeground, #555); font-size: 10px"> ${r.type}</span>
            </div>`
        )
        .join("");
      ...
      searchResults.innerHTML =
        '<div class="search-result-item">Search failed</div>';
    }
  }, 300);
});
...
const communityToggle = document.getElementById(
  "community-color-toggle"
) as HTMLInputElement;
